...
setProperty = ({element, attributeName}, value) => {
  console.log('setting property')
  element[attributeName.replace(/^./, '')] = value;
},
  ...
  processCallback(instance, parts, state) {
    ...
    for (part of parts) {
      if ((v = part.eval(values)) !== part.value) {
        ...
        // <x attr={{single}}>
        else part.attributeName.startsWith('.') ? setProperty(part, v) : 
          part.setter.parts.length === 1 ? prop(part.element, part.attributeName, part.value = v) : 
          part.value = v
      }
    }
  }
